...
const InteractiveDashboard: React.FC<Props> = ({ lang }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10 md:space-y-20 font-host w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {content.market.map((m, idx) => (
          <div key={idx} className="glass-card border border-white/10 p-8 md:p-10 rounded-2xl md:rounded-3xl relative overflow-hidden group hover:border-[#FF8A00] transition-all duration-500">
            <p className="text-gray-600 text-[8px] md:text-[9px] font-black uppercase tracking-[0.4em] mb-3 md:mb-5">{m.label}</p>
            <p className="text-3xl md:text-5xl font-black text-white mb-2 tracking-tighter">{m.value}</p>
      ...
      {/* Chart Section */}
      <div className="bg-black border border-white/10 p-6 md:p-12 rounded-[2rem] md:rounded-[3rem] shadow-2xl overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 gap-5 md:gap-8">
          <div>
            <h3 className="text-xl md:text-3xl font-black text-white mb-2 uppercase tracking-tighter">{content.dashboard.chartTitle}</h3>
          ...
          <div className="flex gap-4 md:gap-6 text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] bg-white/[0.03] px-4 md:px-5 py-2 md:py-3 rounded-full border border-white/5">
