...
const OnePager: React.FC<Props> = ({ lang }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-12 md:space-y-24 px-4 sm:px-8 font-host overflow-hidden pb-16 md:pb-32">
      ...
      {/* Hero Section */}
      <section className="relative pt-12 pb-10 md:pt-40 md:pb-24 overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto text-center relative z-10 px-2">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6 md:mb-8 shadow-inner backdrop-blur-md">
            ...
            <span className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-white/80">{content.round}</span>
          </div>
          
          <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white leading-[0.9] tracking-tighter uppercase mb-6 md:mb-8 text-balance">
          ...
          <p className="text-sm md:text-lg text-gray-400 font-medium leading-relaxed max-w-3xl mx-auto mb-8 md:mb-10 opacity-90 uppercase tracking-tight">
            {content.opportunity}
          </p>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-12 items-start pt-6">
        <div className="lg:col-span-7 space-y-8 md:space-y-10 order-2 lg:order-1">
            <h2 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter leading-none text-center lg:text-left">
              {lang === 'es' ? 'VISIÓN GENERAL DEL DEAL' : 'DEAL OVERVIEW'}
            </h2>
            <div className="space-y-6 md:space-y-8">
              <p className="text-xs md:text-base text-gray-400 font-medium leading-relaxed uppercase tracking-widest opacity-80 border-l-2 border-[#FF8A00] pl-4 md:pl-6">
                {lang === 'es' 
                  ? "VaultBit opera bajo un modelo de 'Storage as a Service' inexpugnable, combinando infraestructura física Grado VII con especialización técnica para eliminar el error humano en la custodia de activos."
                  : "VaultBit operates under an impregnable 'Storage as a Service' model, combining Grade VII physical infrastructure with technical specialization to eliminate human error in asset custody."}
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
                  {content.market.map((m, i) => (
                      <div key={i} className="p-6 md:p-8 bg-white/5 border border-white/10 rounded-2xl md:rounded-3xl group hover:border-[#FF8A00]/50 transition-all">
                          <p className="text-[8px] md:text-[9px] text-[#FF8A00] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] mb-2 md:mb-3">{m.label}</p>
                          <p className="text-xl md:text-2xl font-black text-white">{m.value}</p>
                      </div>
                  ))}
              </div>
            </div>
        ...
        <div className="lg:col-span-5 bg-white text-black p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl space-y-5 md:space-y-6 relative order-1 lg:order-2">
            ...
            <div className="space-y-3 md:space-y-4">
                {[
                  ...
                  <div key={idx} className="flex justify-between items-center text-gray-500">
                      ...
                      <span className="text-xs md:text-sm font-black text-black text-right">{item.value}</span>
                  </div>
                ))}
      ...
      <section className="space-y-8 md:space-y-12 pt-10 border-t border-white/5">
        <div className="max-w-3xl text-center lg:text-left mx-auto lg:mx-0">
          <h2 className="text-2xl md:text-4xl font-black text-white uppercase tracking-tighter leading-none mb-3 md:mb-5">
            {comp.title}
          </h2>
          <p className="text-xs md:text-base text-gray-400 font-medium uppercase tracking-widest opacity-80">
            {comp.subtitle}
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px]">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="py-5 md:py-6 px-4 text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-gray-600 w-1/4">{comp.headers[0]}</th>
                  {comp.headers.slice(1).map((h, i) => (
                    <th key={i} className="py-5 md:py-6 px-4 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-gray-600 text-center">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {comp.rows.map((row, i) => (
                  <tr key={i} className={`border-b border-white/5 transition-colors ${row.name === 'VaultBit' ? 'bg-[#FF8A00]/5' : 'hover:bg-white/[0.02]'}`}>
                    <td className={`py-5 md:py-6 px-4 font-black uppercase tracking-widest text-[10px] md:text-xs ${row.name === 'VaultBit' ? 'text-[#FF8A00]' : 'text-white'}`}>
                      {row.name}
                    </td>
                    {row.values.map((val, j) => (
                      <td key={j} className="py-5 md:py-6 px-4 text-center">
                        <div className="flex justify-center">
                          {val ? (
                            <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full flex items-center justify-center ${row.name === 'VaultBit' ? 'bg-[#FF8A00]' : 'bg-green-500/20'}`}>
                              <svg className={`w-3 h-3 md:w-3.5 md:h-3.5 ${row.name === 'VaultBit' ? 'text-black' : 'text-green-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"></path>
                              </svg>
                            </div>
                          ) : (
                            <div className="w-5 h-5 md:w-6 md:h-6 rounded-full bg-red-500/10 flex items-center justify-center">
                              <svg className="w-3 h-3 md:w-3.5 md:h-3.5 text-red-500/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {/* Highlights Section */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 border-t border-white/5 pt-12 md:pt-20">
        {content.financials.map((metric, i) => (
          <div key={i} className="text-center sm:text-left group bg-white/[0.01] p-6 rounded-2xl border border-white/5 md:bg-transparent md:p-0 md:border-0">
            <p className="text-[9px] md:text-[10px] text-gray-600 uppercase tracking-[0.3em] md:tracking-[0.5em] font-black mb-3 md:mb-5 group-hover:text-[#FF8A00] transition-colors">{metric.label}</p>
            <p className="text-3xl md:text-5xl font-black text-white tracking-tighter mb-2 md:mb-3 leading-none">{metric.value}</p>
            <p className="text-[9px] md:text-[11px] text-white/40 font-black uppercase tracking-[0.2em]">{metric.description}</p>
          </div>
        ))}
      </section>
    </div>
